...
export default function SSHTunnel({ config, setConfig, saveConfig }) {
  ...
  // Show tunnel mapping from channels
  const tunnelChannels = (config?.channels || []).filter(ch => ch.tunnel?.enabled);

  return (
    <div className="p-6 max-w-3xl">
      <h1 className="text-2xl font-bold mb-6">SSH Tunnel</h1>

      <div className="bg-dark-800 rounded-lg border border-dark-700 p-5 space-y-4">
        <h2 className="text-sm font-medium text-dark-300 uppercase tracking-wider">Connection Settings</h2>

        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2">
            <label className="block text-xs text-dark-400 mb-1.5">Host</label>
            <input
              type="text"
              value={ssh.host || ''}
        ...
        <div>
          <label className="block text-xs text-dark-400 mb-1.5">Username</label>
          <input
            type="text"
            value={ssh.username || ''}
        ...
        <div>
          <label className="block text-xs text-dark-400 mb-1.5">Authentication</label>
          <div className="space-y-3">
            <div>
              <label className="block text-xs text-dark-500 mb-1">Private Key Path</label>
            ...
            <div className="text-center text-xs text-dark-500">— or —</div>
            <div>
      ...
      {/* Port mappings from channels */}
      {tunnelChannels.length > 0 && (
        <div className="mt-6">
          <h2 className="text-lg font-semibold mb-3">Tunnel Port Mappings</h2>
          <div className="bg-dark-800 rounded-lg border border-dark-700 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-dark-400 text-xs uppercase border-b border-dark-700">
                  <th className="text-left px-4 py-3">Channel</th>
                  <th className="text-center px-4 py-3">Local Port</th>
                  <th className="text-center px-4 py-3"></th>
                  <th className="text-center px-4 py-3">Remote Port</th>
                </tr>
              </thead>
              <tbody>
          ...
          <p className="text-xs text-dark-500 mt-2">
            Port mappings are configured per-channel in the Channels page.
          </p>
        </div>
      )}
      ...
      {tunnelChannels.length === 0 && (
        <div className="mt-6 bg-dark-800/50 rounded-lg border border-dark-700 p-6 text-center">
          <div className="text-dark-400 text-sm">
            No channels have SSH tunnel enabled. Enable tunnel in channel settings to see port mappings here.
          </div>
        </div>
      )}
    </div>
